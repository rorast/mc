    ...
    <div class="page-container">
        <div class="mt-20">
            <table class="table table-border table-bordered table-hover table-bg table-sort">
                ...
                <tr class="text-c">
                    <th width="60">编号</th>
                    <th width="160">富贵鸡编码</th>
                    <th width="100">购买者手机号</th>
                    <th width="250">购买时间</th>
                    <th width="80">过期时间</th>
                    <th width="80">产卵个数</th>
                    <th width="80">生命周期已过/天</th>
                    <th width="80">操作</th>
                ...
                <?php if(is_array($log)): $i = 0; $__LIST__ = $log;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                        ...
                        <td><?php echo ($vo["animal_on"]); ?></td>
                        <td><?php echo ($vo["phone"]); ?></td>
                        <td><?php echo (date("Y-m-d H:i",$vo["create_time"])); ?></td>
                        <td><?php echo (date("Y-m-d H:i",$vo["expired_time"])); ?></td>
                        <td><?php echo ($vo["hatchery"]); ?> 个</td>
                        <td><?php echo (pastDay(time(),$vo["create_time"])); ?> 天</td>
                        <td class="td-status">
    ...
    <script type="text/javascript">
        ...
        function no_pass(obj,id){
            ...
            layer.confirm('确认要删除吗吗？',function(index){
                ...
                $.post("<?php echo U('Tool/Buy/delAnimal');?>",{id:id},function(data){
                    if(data.status == 1){
                        $(obj).parents("tr").remove();
